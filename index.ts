...
interface RankData {
  [rank: string]: RankEntry;
}

interface ImprovementRecord {
  eventId: string;
  format: 'single' | 'average';
  oldTime: number;
  newTime: number;
  score: number;
}
...
async function loadWCIF(filePath: string): Promise<WCIF> {
  const data = fs.readFileSync(path.resolve(filePath), 'utf8');
  const wcif = JSON.parse(data) as WCIF;
  const improvements: ProcessedResult[] = [];
  ...
  for (const event of wcif.events) {
    ...
    for (const round of event.rounds) {
      for (const result of round.results) {
        ...
        const attempts = result.attempts.map((a: any) => a.result).filter((r: number) => r > 0);
...
async function fetchRankingData(eventId: string, format: 'single' | 'average'): Promise<RankData> {
  const url = `https://hamza-yh.github.io/wcaranks/api/${format}/${eventId}.json`;
  const res = await fetch(url);
  if (!res.ok) throw new Error(`Failed to fetch data for ${eventId} (${format})`);
  return res.json();
}

function timeToRank(time: number, data: RankData): { rank: number; total: number } {
...
function weightCompetitors(total: number): number {
  return Math.log10(total + 1);
}

async function analyzeImprovements(wcif: WCIF): Promise<Map<number, PersonScore>> {
  const scores = new Map<number, PersonScore>();
  const rankingCache = new Map<string, RankData>();

  const bestsMap = new Map<number, Map<string, { single?: PersonalBest; average?: PersonalBest }>>();
  for (const person of wcif.persons) {
    const personBests = new Map<string, { single?: PersonalBest; average?: PersonalBest }>();
    for (const pb of person.personalBests) {
      if (!personBests.has(pb.eventId)) personBests.set(pb.eventId, {});
      personBests.get(pb.eventId)![pb.type] = pb;
    }
    bestsMap.set(person.registrantId, personBests);
  }

  async function getRankings(eventId: string, format: 'single' | 'average') {
  ...
  for (const result of wcif.processedResults || []) {
    ...
    if (!Object.keys(rankings).length) continue;
